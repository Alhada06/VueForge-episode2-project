...
//   status.value = "loaded";
// }
const {state,isLoading,execute,isReady}=useAsyncState(()=>deskree.reviews.get(props.productId),null,{immediate:false});
const reviews=computed(()=>{
  return state.value?.data.map((review)=>({...review.attributes}))
})
const loadReviews=()=>{
    execute();
...
<template>
  <div>
    ...
    <button @click="loadReviews" class="underline flex items-center">
      ...
      <AppSpinner class="ml-3" size="sm" v-if="isLoading" />
    </button>
  </div>

  <div class="my-10" v-if="isReady">
    ...
    <button v-if="reviews.length" class="underline my-5" @click="handleShowReviewsToggle">
